...
if (array_key_exists('s', $_GET) && !is_admin()) {
	add_action('wp_head', function () {
		echo '
<script>
function mounted_hook() {
this.show_global_search();
this.global_search_query = "' . $_GET['s'] . '";
this.global_search_post = true;
this.global_search_gallery = true;
this.global_search();
}
</script>
';
...
function pf_post_ding($id)
{
	...
	setcookie('bigfa_ding_' . $id, $id, $expire, '/; samesite=strict', $domain, false);
...
"charactor":"' . $charactor . '"
	...
	$result = wp_remote_post($url, array("body" => $info));
...
<p>您曾在《' . get_the_title($comment->comment_post_ID) . '》的留言:<br />' . do_shortcode(trim(get_comment($parent_id)->comment_content)) . '</p>
<p>' . trim($comment->comment_author) . ' 给你的回复:<br />' . do_shortcode(trim($comment->comment_content)) . '<br /></p>
...
function pf_escape_comment_tags($comment_text)
{
	$comment_text = str_ireplace('<', '&lt;', $comment_text);
	$comment_text = str_ireplace('&lt;img', '<img', $comment_text);
	$comment_text = str_ireplace('&lt;pre', '<pre', $comment_text);
	$comment_text = str_ireplace('&lt;/pre', '</pre', $comment_text);
	return $comment_text;
}
...
function pre_validate_comment_span($commentdata)
{
	if (!is_admin() & !wp_verify_nonce($_POST['wp_nonce'], 'wp_rest')) {
		wp_die('
<p></p>
<p>WP NONCE验证失败，判定为机器人恶意发送的垃圾评论！如果启用了“缓存”，则无法正常获取NONCE，因此也可能会判定为垃圾评论。若此操作是正常操作，请停用任何网站缓存功能。</p>
<p></p>
